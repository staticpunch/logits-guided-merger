...
class Mask(nn.Module):
    ...
    def _check_shape_compatibility(self):
        ...
        try:
            if self.config.mode in ("scalar", "vector_input"):
                in_test = torch.rand(self.size)
                out_test = self.weight * in_test
            elif self.config.mode in ("vector_output"):
                in_test = torch.rand(self.size)
                out_test = (self.weight * in_test.T).T
...
class LinearsWithMasks(ModulesWithMasks):
    ...
    def forward(self, x):
        ...
        masked_biases = [
            b_mask * linear.bias if (linear.bias is not None and b_mask is not None)
            else linear.bias
            for b_mask, linear in zip(constrained_bias_masks, self.linears)
        ]
        
        if all(b is None for b in masked_biases):
            merged_bias = None
        else:
            merged_bias = sum(
                b if b is not None 
                else torch.zeros_like(self.linears[0].weight[:, 0])
        ...
        with torch.no_grad():
            for i, linear in enumerate(self.linears):
                output_i = torch.nn.functional.linear(x, linear.weight, None)
                partial_outputs.append(output_i)
                
        merged_output = sum(
            [mask * output_i for mask, output_i 
             in zip(constrained_weight_masks, partial_outputs)]
        )
        
        if merged_bias is not None:
            merged_output = merged_output + merged_bias
    ...
    def forward(self, input_ids):
        ...
        with torch.no_grad():
            for i, emb in enumerate(self.embeddings):
                output_i = emb(input_ids)
                partial_outputs.append(output_i)
                
        merged_output = sum(
            [mask * output_i for mask, output_i 
             in zip(constrained_masks, partial_outputs)]
        )
        return out
