...
class MergerDataCollator:
    """Custom data collator for merged model training."""
    
    tokenizer: PreTrainedTokenizerBase
    pad_to_multiple_of: Optional[int] = None
    return_tensors: str = "pt"

    def pad_without_warning(self, *pad_args, **pad_kwargs):
        """Pad without fast tokenizer warning."""
        ...
        try:
            padded = self.tokenizer.pad(*pad_args, **pad_kwargs)
        finally:
    ...
    def __call__(self, examples):
        ...
        for example in examples:
            _ = example.pop("attention_mask")
            inputs_ids.append({"input_ids": example.pop("input_ids")})
            data_sources.append(example.pop("data_source"))
            
        batch = self.pad_without_warning(
            inputs_ids,
            return_tensors="pt",
        ...
        # Convert data source to tensor
        batch["data_source"] = torch.tensor(data_sources, dtype=torch.long)
